...
check_tool_prerequisites() {
	for preReq in ${1}; do
		if ! is_installed $preReq; then
			warn "$preReq not installed or not in current path.  Please install to continue configuring your environment and re-run $TOOLNAME"
			return 1
		fi
	done
}
...
#3
log "Checking tool/utility pre-requisites..."
check_tool_prerequisites "astyle cppcheck" || err "One or more required tools not found.  Complete installation of tool prequisites and re-run $TOOLNAME"
...
log "Your environment is configured for MySensors development... Thanks for your support!"
